...
export default function DashboardPage() {
  ...
  const filteredCustomers = useMemo(() => {
    return customers.filter(customer => {
      ...
      return matchesSearch && matchesCity && matchesStatus && matchesAge
    })
  }, [searchQuery, cityFilter, statusFilter, ageFilter, customers])
              ...
              filteredCustomers.map((customer) => (
                <TableRow key={customer.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <TableCell>
                    <Badge className={cn(
                      "font-medium",
                      customer.status === 'Active' ? "bg-emerald-50 text-emerald-700 border-emerald-200 hover:bg-emerald-50" : 
                      customer.status === 'Pending' ? "bg-amber-50 text-amber-700 border-amber-200 hover:bg-amber-50" : 
